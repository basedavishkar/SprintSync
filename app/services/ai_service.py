...
Make it practical and actionable. Use plain text only, no markdown formatting or asterisks."""
    ...
    def generate_daily_plan(self, username: str, user_tasks: list = None) -> str:
        ...
        try:
            ...
            if user_tasks and len(user_tasks) > 0:
                ...
                sorted_tasks = sorted(user_tasks, key=lambda x: priority_order.get(x['status'], 4))
                
                task_context = "Based on your current tasks (prioritized by status):\n"
                for i, task in enumerate(sorted_tasks[:5], 1):  # Show top 5 prioritized tasks
                    status_emoji = "🔄" if task['status'] == 'in_progress' else "📋" if task['status'] == 'todo' else "✅"
...
Focus on balanced portions and hydration. Track progress weekly."""
        ...
        elif any(word in title_lower for word in ["workout", "exercise", "gym"]):
